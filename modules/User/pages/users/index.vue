<script setup lang="ts">
// @ts-nocheck
//import useIndex from "@/modules/User/composables/useIndex";
//import AppPaginationB from "@/components/AppPaginationB.vue";
//import AppPageHeader from "@/components/AppPageHeader.vue"
//import AppBtn from "@/components/AppBtn.vue"
//import IconAvatar from "@/icons/Avatar.vue"

definePageMeta({
  layout: "custom",
});

const {
  errors,
  data,
  router,

  deleteRow,
  setSearch,
  setSort  
} = useIndex()

</script>

<template>
  <div>
    <AppPageHeader> Usuarios </AppPageHeader>

    <div class="flex space-x-2">
      <!--NuxtLink class="btn btn-primary" to="/users/create">
        <span>Crear</span>
      </NuxtLink-->
    </div>    
    <div class="overflow-hidden panel mt-6">      
      <div class="mb-6 flex justify-between items-center">
        <div class="flex items-center">
          <div class="flex w-full bg-white shadow rounded">
            <input
              class=""
              type="text"
              v-model="data.search"
              @input="setSearch"
              placeholder="Filtrar…"
            />
          </div>
        </div>
      </div>      
      <div class="table-data__wrapper">
        <table class="table-data">
          <thead>
            <tr class="">
              <th class="">
                Avatar
              </th>
              <th class="">
                <NuxtLink to="#" @click.prevent="setSort('name')">Nombre</NuxtLink>
              </th>
              <th class="">
                <NuxtLink to="#" @click.prevent="setSort('email')">Correo</NuxtLink>
              </th>
              <th class="">
                <NuxtLink to="#" @click.prevent="setSort('role')">Role</NuxtLink>
              </th>
              <th class="">Acción</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="row in data.rows" :key="row.id" class="">
              <td>
              <div class="inline-flex items-center space-x-2">
            <img
              v-if="row.avatar"
              :src="row.avatar"
              class="w-10 h-10 rounded-full"
              alt=""
            />
            <IconAvatar class="w-10 h-10 text-gray-400 rounded-full" v-else />            
          </div>
              </td>
              <td class="">
                <!--NuxtLink
                  class="text-indigo-600 hover:text-indigo-800 underline"
                  :to="{ name: 'userEdit', params: { id: row.id }}"                  
                >
                  {{ row.name }}
                </NuxtLink-->
                {{ row.name }}
              </td>
              <td class="">
                {{ row.email }}
              </td>
              <td class="">
                {{ row.role_id }}
              </td>
              <td class="">
                <div class="flex items-center space-x-1">                
                  <!--AppBtn
                    class="btn btn-primary btn-xs"                    
                    @click="router.push({ path: '/users/edit/'+row.id })"
                  >
                    Editar
                  </AppBtn>
                  <AppBtn
                    @click="deleteRow(row.id)"                    
                    class="btn btn-secondary btn-xs"                    
                  >
                    Eliminar
                  </AppBtn-->
                  
                  <NuxtLink
                    class="btn btn-primary btn-xs mr-1"                    
                    :to="`/users/edit/${row.id}`"
                  >
                    Editar
                  </NuxtLink>
                  <AppBtn
                    @click=""                    
                    class="btn btn-danger btn-xs ml-1"                    
                  >
                    Eliminar
                  </AppBtn>
                </div>
              </td>
            </tr>
            <tr v-if="data.rows.length === 0">
              <td class="" colspan="4">Usuarios no encontrados.</td>
            </tr>
          </tbody>
        </table>
      </div>
      <span v-if="Object.keys(errors).length > 0" class="text-red-500">{{ errors }}</span>
      <AppPaginationB v-if="data.links" :links="data.links" />      
    </div>
  </div>
</template>
